...
g3_findings_report() {
    ...
    # (a) report exists, version line == v1.4; (b) CM baseline SHA pinned;
    ...
    grep -qE '^Version: v1\.4|^- Version: v1\.4' "$file" \
        || fail "report version line is not v1.4"
    pass "report version v1.4"
    ...
    in_table && /^\| F-[0-9]+ \|/ {
        ...
        if (ver != "v1.4") { bad(id, "version not v1.4: '" ver "'"); next }
