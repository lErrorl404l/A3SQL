...
g3_findings_report() {
    ...
    # (a) report exists, version line == v1.3; (b) CM baseline SHA pinned;
    ...
    grep -qE '^Version: v1\.3|^- Version: v1\.3' "$file" \
        || fail "report version line is not v1.3"
    pass "report version v1.3"
    ...
    in_table && /^\| F-[0-9]+ \|/ {
        ...
        if (ver != "v1.3") { bad(id, "version not v1.3: '" ver "'"); next }
