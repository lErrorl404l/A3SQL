...
g3_findings_report() {
    ...
    # (a) report exists, version line == v1.2; (b) CM baseline SHA pinned;
    ...
    grep -qE '^Version: v1\.2|^- Version: v1\.2' "$file" \
        || fail "report version line is not v1.2"
    pass "report version v1.2"
    ...
    in_table && /^\| F-[0-9]+ \|/ {
        ...
        if (ver != "v1.2") { bad(id, "version not v1.2: '" ver "'"); next }
