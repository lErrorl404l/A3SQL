...
| Category | Features |
|---|---|
| **Schema** | CREATE/DROP TABLE, CREATE TABLE AS SELECT (CTAS), CREATE/DROP INDEX (BTREE, TRIGRAM), CREATE/DROP VIEW, ALTER TABLE ADD/DROP/RENAME COLUMN, ALTER TABLE RENAME, TRUNCATE, VACUUM, REINDEX |
...
| **SELECT** | WHERE (`= <> < <= > >=`), AND/OR/NOT, IN, NOT IN, BETWEEN, LIKE/NOT LIKE, REGEXP/NOT REGEXP, MATCH, IS NULL/IS NOT NULL, EXISTS, CASE WHEN, CAST, `%%` fuzzy match, subqueries, derived tables, DISTINCT |
| **JOINs** | CROSS, INNER, LEFT, FULL OUTER, NATURAL, USING, ON (incl. subqueries) |
| **Aggregates** | COUNT (incl. DISTINCT and NULL-skipping), SUM, AVG, MIN, MAX, GROUP_CONCAT — with GROUP BY, HAVING, aggregate `FILTER (WHERE ...)`, ORDER BY, LIMIT/OFFSET |
...
| **Functions** | UPPER, LOWER, LENGTH, SUBSTR, TRIM, LTRIM, RTRIM, CONCAT, COALESCE, IFNULL, NULLIF, ROUND, ABS, CEIL, FLOOR, INSTR, CHAR, TYPEOF, match_search, NOW(), CURRENT_TIMESTAMP/CURRENT_DATE/CURRENT_TIME, datetime()/date()/time()/strftime() with modifiers, GROUP_CONCAT |
...
`CREATE TABLE AS SELECT` (CTAS) creates a table from a query's result set.
Column types and names come from the SELECT; primary keys are not carried
over:

```sql
CREATE TABLE heavy_weapons_copy AS SELECT * FROM weapons WHERE barrelLength > 400.0;
CREATE TABLE caliber_list AS SELECT DISTINCT caliber FROM weapons;
```
...
**REGEXP** — wildcard pattern matching. `*` matches any sequence, `?` matches
one character. Unlike LIKE, `REGEXP` does not use the trigram index:

```sql
SELECT * FROM weapons WHERE name REGEXP 'M4*';       -- names starting M4
SELECT * FROM weapons WHERE name REGEXP '*a1';       -- names ending a1
SELECT * FROM weapons WHERE name NOT REGEXP 'AK*';
```

**MATCH** — case-insensitive substring search. Equivalent to a `%needle%`
LIKE, but reads clearly and ignores case:

```sql
SELECT * FROM weapons WHERE name MATCH 'm4';         -- matches M4A1, RHS_M4...
```
...
-- FILTER (WHERE ...) restricts which rows feed an aggregate, without a
-- GROUP BY / HAVING rewrite
SELECT COUNT(*) FILTER (WHERE barrelLength > 400.0) AS heavy_count FROM weapons;
SELECT caliber,
       COUNT(*) FILTER (WHERE barrelLength > 400.0) AS heavy,
       COUNT(*) FILTER (WHERE barrelLength <= 400.0) AS light
FROM weapons GROUP BY caliber;
```

`MIN` and `MAX` ignore NULL values: a column of `{10, NULL, 5}` returns 5 and
10 respectively, not NULL.
...
SELECT ROUND(barrelLength, 1), ABS(barrelLength), CEIL(barrelLength), FLOOR(barrelLength) FROM weapons;
SELECT NULLIF(barrelLength, 0.0) FROM weapons;
...
- `CEIL` / `FLOOR` — round up / down to the nearest integer (returned as a float).
