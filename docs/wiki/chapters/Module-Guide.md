...
Adding a3sql as a dependency gives your mod a shared, in-memory SQL database
inside the server: store player stats, loadouts, event logs, or mission state
with plain SQL, persist it across restarts, and read or write the same tables
from any other a3sql-integrated mod. No external database to install, no
PBO-side parsing. Just `call` an SQF function.

This guide covers integration for modders and scripters: dependency setup, the
SQF function API, table-naming conventions, the security model, and a worked
example.
...
All functions take an optional extension name as the last parameter (defaults
to `"a3sql"`). They return a parsed result array `[code, "status", data]` where
`code == 0` means success.

Five frequently used functions have short `a3sql_fnc_*` aliases; everything
else uses the full `a3sql_database_fnc_*` name:
...
| `a3sql_database_fnc_executePrepared` | Run a prepared statement by name with params |
| `a3sql_database_fnc_executeTimed` | Same as execute, logs to RPT if query takes >10ms |
| `a3sql_database_fnc_prepare` | Prepare a named statement for repeated use |
...
| Function | Description |
|---|---|
| `a3sql_database_fnc_save` | Save full database to binary file |
| `a3sql_database_fnc_load` | Restore full database from binary file |
| `a3sql_database_fnc_exportJSON` | Export a table as JSON |
| `a3sql_fnc_exportCSV` | Export a table as CSV |
| `a3sql_database_fnc_exportSQL` | Export the full database as SQL dump |
| `a3sql_database_fnc_loadJSON` | Load JSON data into a table |
| `a3sql_database_fnc_dumpSQL` | Alias for `exportSQL` |

### Initialize and settings

| Function | Description |
|---|---|
| `a3sql_database_fnc_init` | Initialize the extension: print version to RPT, push TCP credentials, start the listener if enabled |

CBA settings are registered automatically at preInit, and the auto-save /
auto-load hooks are wired up at postInit. Your mod does not call those steps
itself. See [CBA Settings](CBA-Settings.md).
...
// Prepared statement
["get_player", "SELECT name, score FROM players WHERE uid = $1"] call a3sql_database_fnc_prepare;
_result = ["get_player", ["76561198000000001"]] call a3sql_database_fnc_executePrepared;

// Persistence
["mydata.bin"] call a3sql_database_fnc_save;
["mydata.bin"] call a3sql_database_fnc_load;
```

## 2.5 Addon reference

The project follows ACE3 conventions with modular, single-responsibility addons:

| Addon | Function prefix | Purpose |
|-------|----------------|---------|
| `a3sql_admin` | `a3sql_admin_fnc_*` | Server command execution + player tracking |
| `a3sql_analytics` | `a3sql_analytics_fnc_*` | Perf monitoring, kill/shoot events, replay snapshots |
| `a3sql_database` | `a3sql_fnc_*` | Core SQL execution, persistence, export |
| `a3sql_loadouts` | `a3sql_loadouts_fnc_*` | Faction/role loadout templates |
| `a3sql_main` | — | Core defines, version, macros |
| `a3sql_patch_core` | `a3sql_patch_core_fnc_*` | Dynamic patching engine, handlers |
| `a3sql_patch_editor` | `a3sql_patch_editor_fnc_*` | In-game rule editor UI |
| `a3sql_patch_operators` | `a3sql_patch_operators_fnc_*` | Value transformer operators |
| `a3sql_persistence` | `a3sql_persistence_fnc_*` | Player state save/restore on DC/JIP |
| `a3sql_progression` | `a3sql_progression_fnc_*` | Rank/score bridge via existing Arma systems |
...
- Both mods only depend on `a3sql_database`
...
- `LOGIN` is required before any query, and authentication is fail-closed:
  with no credentials configured, LOGIN can never succeed, and every query is
  rejected with `ERR_AUTH` until a successful LOGIN
- Credential comparison is constant-time, so timing side channels cannot leak
  the password
- Change the bind address to `0.0.0.0` only if you need remote access across a
  network (see the standalone server `--bind 0.0.0.0` option)
...
- Clean up old data: `DELETE FROM events_shots WHERE created_at < datetime('now', '-30 days')` — SQLite-style modifiers supported
- Batch INSERTs when importing large datasets: use multi-VALUES format
- Responses over 30 KB fail loudly with a cursor hint — page large result sets with `cursor create <name> <query>` + `cursor fetch <name> [limit]` (LIMIT/OFFSET alone cannot shrink a single response past the cap)
