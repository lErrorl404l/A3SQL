...
The patch framework lets your mission adjust in-game values at runtime from SQL
rules: weapon stats, vehicle fuel and damage, textures and materials, unit
skills, anything you can reach from SQF. Rules live in the `patch_rules` table
and apply on the next frame. No mission restart, no config.cpp compatibility
patches, no PBO rebuild. Because the rules are data, they can be written by the
mission, by the in-game editor, or remotely over the TCP listener, and they
survive restarts.
...
via a dirty-flag on the patch namespace. When new rules are detected,
...
The patch system is split into three focused addons following ACE3 conventions:

| Addon | Function prefix | Purpose |
|-------|----------------|---------|
| `a3sql_patch_core` | `a3sql_patch_core_fnc_*` | Core engine: rule application, PerFrame handler, target handlers, settings |
| `a3sql_patch_editor` | `a3sql_patch_editor_fnc_*` | In-game rule editor dialog (`createDialog "a3sql_patch_editor"`) |
| `a3sql_patch_operators` | `a3sql_patch_operators_fnc_*` | Value transformer functions (opAdd, opMul, etc.) |
...
use init.sqf, and insert a rule directly:
...
The PerFrame handler picks up the new rule on its next cycle and applies it.

The `live_patch <target> <property> <value>` command is a shorthand that
inserts a rule with `match_value=''`, `match_type='exact'`, and
`operator='set'`. Because an empty `match_value` matches nothing under
`match_type='exact'`, classname-targeted rules should use the direct INSERT
above (or the in-game editor); `live_patch` is useful as a stub you then edit.
...
periodically checks for new rules via a dirty-flag on the patch namespace
(`a3sql_patch_namespace`, key `dirty`). When new rules are detected, `fn_applyAll`
reads them in batches (LIMIT 50 pagination), iterates each wrapped in
`try {} catch {}`, and applies the patch via target collection scoping,
match-type filtering, and operator transforms.
...
2. **PerFrame tick** -- Checks the dirty-flag on the patch namespace. If dirty
   (or after 60s timeout), calls `fn_applyAll`.
...
```sqf
_result = [] call a3sql_patch_core_fnc_applyAll;
// [0, "OK", [12, 1]]  -- 12 applied, 1 error
```

#### a3sql_patch_core_fnc_applyRule
...
```sqf
// From hashmap
_result = [_ruleHashmap] call a3sql_patch_core_fnc_applyRule;

// From raw DB row
_result = [dbRowArray] call a3sql_patch_core_fnc_applyRule;
```

#### a3sql_patch_core_fnc_applyByTarget
...
```sqf
_result = ["vehicle", "B_MBT_01_cannon_F"] call a3sql_patch_core_fnc_applyByTarget;
```

#### a3sql_patch_core_fnc_getRule
...
```sqf
_result = [3] call a3sql_patch_core_fnc_getRule;
...
#### a3sql_patch_core_fnc_listRules
...
```sqf
// List all rules
_rules = [] call a3sql_patch_core_fnc_listRules;

// Search by name
_rules = ["btr"] call a3sql_patch_core_fnc_listRules;

// Filter by hashmap
_rules = [createHashMapFromArray [
    ["target_type", "vehicle"], ["active", 1]
]] call a3sql_patch_core_fnc_listRules;
```

#### a3sql_patch_core_fnc_deleteRule
...
```sqf
_result = [3] call a3sql_patch_core_fnc_deleteRule;
```

#### a3sql_patch_core_fnc_setDirty
...
// Clear dirty flag
[false] call a3sql_patch_core_fnc_setDirty;
```

#### a3sql_patch_core_fnc_reload
...
#### a3sql_patch_core_fnc_registerHandler
...
}] call a3sql_patch_core_fnc_registerHandler;
```
...
```sqf
_result = [5, "3"] call a3sql_patch_core_fnc_opAdd;  // 8
```

#### a3sql_patch_core_fnc_opSub
...
```sqf
_result = [10, "3"] call a3sql_patch_core_fnc_opSub;  // 7
```

#### a3sql_patch_core_fnc_opMul
...
```sqf
_result = [4, "1.5"] call a3sql_patch_core_fnc_opMul;  // 6
```

#### a3sql_patch_core_fnc_opDiv
...
```sqf
_result = [10, "2"] call a3sql_patch_core_fnc_opDiv;  // 5
_result = [10, "0"] call a3sql_patch_core_fnc_opDiv;  // 0
```

#### a3sql_patch_core_fnc_opMod
...
```sqf
_result = [10, "3"] call a3sql_patch_core_fnc_opMod;  // 1
```

#### a3sql_patch_core_fnc_opCat
...
```sqf
_result = ["hello", " world"] call a3sql_patch_core_fnc_opCat;  // "hello world"
```

#### a3sql_patch_core_fnc_opDefault
...
```sqf
_result = [nil, "default"] call a3sql_patch_core_fnc_opDefault;  // "default"
_result = ["existing", "default"] call a3sql_patch_core_fnc_opDefault;  // "existing"
```
...
#### a3sql_patch_core_fnc_handleWeapon
...
```sqf
_affected = ["B_MBT_01_cannon_F", "fuel", "0.5"] call a3sql_patch_core_fnc_handleVehicle;
```

#### a3sql_patch_core_fnc_handleMagazine
...
```sqf
_affected = ["30Rnd_556x45_Stanag", "", "30Rnd_556x45_Stanag_Tracer_Red"] call a3sql_patch_core_fnc_handleMagazine;
```

#### a3sql_patch_core_fnc_handleUnit
...
```sqf
_affected = ["B_soldier_AR_F", "aimingAccuracy", "0.8"] call a3sql_patch_core_fnc_handleUnit;
```

#### a3sql_patch_core_fnc_handleTexture
...
```sqf
_affected = ["Land_WoodenBox_F", "", "a3\data_f\default_texture.paa"] call a3sql_patch_core_fnc_handleTexture;
```

#### a3sql_patch_core_fnc_handleMaterial
...
```sqf
_affected = ["Land_WoodenBox_F", "", "a3\data_f\default.rvmat"] call a3sql_patch_core_fnc_handleMaterial;
```

#### a3sql_patch_core_fnc_handleEntity
...
```sqf
_affected = ["Land_HelipadSquare_F", "myCustomVar", "hello"] call a3sql_patch_core_fnc_handleEntity;
```
...
| **round** | Round the current value to N decimal places | `value` is the precision (default 0 = whole number) |
| **clamp** | Clamp the current value to `[min, max]` | `value` is `"min|max"` (default `"0|100"`) |
| **negate** | Negate a numeric value or invert a boolean string | Flips sign, or `"true"` ↔ `"false"` |
| **replace** | Regex-replace substrings in the current value | `value` is `"search|replacement"` |
| **format** | SQF `format` the current value | `value` is a format string; `%1` = current value, `%2` = target object |
...
| `a3sql_patch_stream_output` | CHECKBOX | false | Emit a chat notification for every rule application (see Real-time Streaming) |
...
```sqf
["weapon", a3sql_patch_core_fnc_handleWeapon] call a3sql_patch_core_fnc_registerHandler;
["vehicle", a3sql_patch_core_fnc_handleVehicle] call a3sql_patch_core_fnc_registerHandler;
["unit", a3sql_patch_core_fnc_handleUnit] call a3sql_patch_core_fnc_registerHandler;
["magazine", a3sql_patch_core_fnc_handleMagazine] call a3sql_patch_core_fnc_registerHandler;
["texture", a3sql_patch_core_fnc_handleTexture] call a3sql_patch_core_fnc_registerHandler;
["material", a3sql_patch_core_fnc_handleMaterial] call a3sql_patch_core_fnc_registerHandler;
["entity", a3sql_patch_core_fnc_handleEntity] call a3sql_patch_core_fnc_registerHandler;
```
...
```sqf
_result = [] call a3sql_patch_core_fnc_listRules;
...
_rule = (["mbt_fuel_boost"] call a3sql_patch_core_fnc_listRules);

// Delete it
[1] call a3sql_patch_core_fnc_deleteRule;  // assuming id=1
```
...
|----------|-------------|
| `a3sql_patch_core_fnc_applyGroup` | Apply all active rules in a group by name |
| `a3sql_patch_core_fnc_activateGroup` | Activate a group and apply its rules |
| `a3sql_patch_core_fnc_deactivateGroup` | Deactivate all rules in a group |

### Example
```sqf
["balance"] call a3sql_patch_core_fnc_activateGroup;
["balance"] call a3sql_patch_core_fnc_deactivateGroup;
["balance"] call a3sql_patch_core_fnc_applyGroup;
```
...
- **After any rule change**: `patch_rules` is saved to binary via `save patch_rules`, debounced to 5 seconds after the last change
