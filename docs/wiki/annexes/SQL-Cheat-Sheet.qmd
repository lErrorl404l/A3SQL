...
-- Create table from a query (CTAS)
...
-- Wildcard pattern (REGEXP): * = any sequence, ? = one char
SELECT * FROM weapons WHERE name REGEXP 'M4*';

-- Case-insensitive substring (MATCH)
SELECT * FROM weapons WHERE name MATCH 'm4';
...
-- Count with aggregate FILTER
SELECT COUNT(*) FILTER (WHERE score > 100) AS veterans FROM players;
```
...
```sql
-- NULL replacement
SELECT COALESCE(score, 0), IFNULL(score, 0) FROM players;

-- NULLIF: NULL when equal, else the first value
SELECT NULLIF(score, 0) FROM players;

-- Rounding
SELECT ROUND(score, 1), ABS(score), CEIL(score), FLOOR(score) FROM players;
```
