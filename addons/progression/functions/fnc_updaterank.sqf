...

{
    ...
    private _killResult = [format ["SELECT COUNT(*) as cnt FROM events_kills WHERE killer_uid = '%1'", [_uid] call a3sql_database_fnc_sqlEscape]] call a3sql_database_fnc_selectMap;
    if (_killResult isNotEqualTo []) then { _kills = (_killResult select 0) get "cnt"; };

    private _deathResult = [format ["SELECT COUNT(*) as cnt FROM events_kills WHERE victim_uid = '%1'", [_uid] call a3sql_database_fnc_sqlEscape]] call a3sql_database_fnc_selectMap;
    if (_deathResult isNotEqualTo []) then { _deaths = (_deathResult select 0) get "cnt"; };

    // ── Escape strings for SQL ──────────────────────────────────
    _name = [_name] call a3sql_database_fnc_sqlEscape;
    private _mission = [missionName] call a3sql_database_fnc_sqlEscape;
    ...
    private _sql = format [
        ...
        [_uid] call a3sql_database_fnc_sqlEscape, _name, [_rank] call a3sql_database_fnc_sqlEscape, _score, _kills, _deaths, [_now] call a3sql_database_fnc_sqlEscape, _mission
    ];
    _sql call a3sql_database_fnc_execute;

    // ── Update highest_rank if current rank is higher ───────────
    private _highestResult = [format ["SELECT highest_rank FROM player_progression WHERE uid = '%1'", [_uid] call a3sql_database_fnc_sqlEscape]] call a3sql_database_fnc_selectMap;
    if (_highestResult isNotEqualTo []) then {
        ...
        if (_storedHighest == "") then {
            [format ["UPDATE player_progression SET highest_rank = '%1' WHERE uid = '%2'", [_rank] call a3sql_database_fnc_sqlEscape, [_uid] call a3sql_database_fnc_sqlEscape]] call a3sql_database_fnc_execute;
        } else {
