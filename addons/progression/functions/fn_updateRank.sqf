...

{
    ...
    private _killResult = [format ["SELECT COUNT(*) as cnt FROM events_kills WHERE killer_uid = '%1'", _uid]] call a3sql_fnc_selectMap;
    if !(_killResult isEqualTo []) then { _kills = (_killResult select 0) get "cnt"; };

    private _deathResult = [format ["SELECT COUNT(*) as cnt FROM events_kills WHERE victim_uid = '%1'", _uid]] call a3sql_fnc_selectMap;
    ...
    private _highestResult = [format ["SELECT highest_rank FROM player_progression WHERE uid = '%1'", _uid]] call a3sql_fnc_selectMap;
    if !(_highestResult isEqualTo []) then {
        ...
        if (_storedHighest == "") then {
            [format ["UPDATE player_progression SET highest_rank = '%1' WHERE uid = '%2'", _rank, _uid]] call a3sql_fnc_execute;
        } else {
