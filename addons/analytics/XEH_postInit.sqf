...
// ── Create tables ──────────────────────────────────────────────
"CREATE TABLE IF NOT EXISTS perf_metrics (id INTEGER PRIMARY KEY, timestamp TEXT, fps FLOAT, fps_min FLOAT, entities INT, vehicles INT, players INT, bandwidth FLOAT, mission_name TEXT)" call a3sql_fnc_execute;
"CREATE TABLE IF NOT EXISTS events_kills (id INTEGER PRIMARY KEY, timestamp TEXT, killer_uid TEXT, killer_unit TEXT, killer_weapon TEXT, killer_pos_x FLOAT, killer_pos_y FLOAT, killer_pos_z FLOAT, victim_uid TEXT, victim_unit TEXT, victim_weapon TEXT, distance FLOAT, headshot INT, mission_name TEXT)" call a3sql_fnc_execute;
"CREATE TABLE IF NOT EXISTS events_shots (id INTEGER PRIMARY KEY, timestamp TEXT, shooter_uid TEXT, weapon TEXT, mission_name TEXT)" call a3sql_fnc_execute;
