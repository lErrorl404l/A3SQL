...
if (_log_level >= 2) then {
    INFO_1("Presets table init: %1",_presetResult);
};
...
if (_log_level >= 2) then {
    INFO("Column migration applied (group_name, notes)");
};
...
addMissionEventHandler ["PlayerConnected", {
    ...
    if (_jip) then {
        if (["a3sql_patch_log_level"] call CBA_fnc_getSetting >= 2) then {
            INFO_2("JIP player %1 (%2) — applying patches",_name,_uid);
        };
        [] call a3sql_patch_core_fnc_applyAll;
    };
}];
...
addMissionEventHandler ["Ended", {
    private _log_level = ["a3sql_patch_log_level"] call CBA_fnc_getSetting;
    if (_log_level >= 2) then {
        INFO("Saving patch rules...");
    };
    "a3sql" callExtension "save patch_rules";
    if (_log_level >= 2) then {
        INFO("Saved OK — patch system cleanup complete");
    };
}];
